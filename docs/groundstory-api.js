...
export class GroundStoryApi {
  ...
  async nearby({
    ...
    if (this.apiUrl) {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            body.message || 'GroundStory API request failed.',
          );
        }
    ...
    if (!this.allowWikimediaFallback) {
      throw new Error(
        'GroundStory API URL is not configured.',
      );
    }

    return this.nearbyFromWikimedia({
      lat,
      lon,
      radiusMiles,
      limit,
  ...
  async nearbyFromWikimedia({
    ...
    url.search = new URLSearchParams({
      ...
      ggsnamespace: '0',
      ggsradius: String(
        Math.min(radiusMiles * 1609.344, 10000),
      ),
      ggslimit: String(
    ...
    const stories = Object.values(
      ...
      return {
        ...
        summary:
          page.extract ||
          'A nearby place with a documented story.',
